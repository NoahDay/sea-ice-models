...
Mean ice thickness
    Breaking points are calculated using a flexural strain failure cr0ion,
...
slider_opts = {
    ...
    "xmax" : [ [0.50 , 4.00 , 10.0, 0.50 ], r"$x_\mathrm{max}$ (km)"                       ],
    "dx"   : [ [0.50 , 1.00 , 10.0, 0.50 ], r"$\Delta{}x$ (m)"                             ],
    "nx"   : [ [1000 , 10000 , 20000, 1000], r"$N_x$ (m)"                                  ],
...
# |-----------------------------------------------------------------------------------------|
 # ND: Changing defaults to match Icepack settings
...
n = len(hist_bins)
min_floe = hist_bins[0]
    
BinLeft = np.zeros(n)
BinRight = np.zeros(n)
floe_rad_c = np.zeros(n-1)

# First left edge is fixed by CICE convention
BinLeft[0] = min_floe
BinRight[0] = BinLeft[0] + hist_bins[0]
floe_rad_c[0] = 0.5 * (BinLeft[0] + BinRight[0])

# Reconstruct remaining bins
for i in range(1, len(floe_rad_c)):
    BinLeft[i] = BinRight[i-1]
    BinRight[i] = BinLeft[i] + hist_bins[i]
    floe_rad_c[i] = 0.5 * (BinLeft[i] + BinRight[i])

# Other constants:
grav = 9.80665
loop_max_iter = 100  # maximum number of iterations to try to reach convergence
...
def x1d(nx_in, dx_in):
    return np.arange(1, nx_in+1) * dx_in
...
if __name__ == "__main__":
    ...
    data_x1d     = x1d(int(slider_opts["nx"][0][1]), slider_opts["dx"][0][1])
    ...
    def stat_str(x, iter=None):
        """Returns a string with statistical values of x and optional iteration count."""
        if len(x) > 0:
            xstr = (f"Range: [{np.min(x):.1f}, {np.max(x):.1f}] m\n"
                    f"Median = {np.median(x):.1f} m\n"
                    f"IQR = {np.subtract(*np.percentile(x, [75, 25])):.1f} m")
            if len(x) > 1:
                xstr += f"\nSkewness = {np.mean(((x - np.mean(x)) / np.std(x))**3):.1f}"
        else:
            xstr = ""

        if iter is not None:
            if iter >= loop_max_iter:
                xstr += f"\nIterations = >{loop_max_iter} (no convergence)"
            else:   
                xstr += f"\nIterations = {iter}"
    ...
    slider_dx   = _create_slider(4, "dx")
    slider_nx   = _create_slider(5, "nx")
    ...
    def update_spectrum(val, phase_random=False, phase_reset=False, in_fracture_loop=False):
        ...
        elif phase_reset and not phase_random:
            data_phase = phase_init * np.ones(nf_max)
        ...
        if not in_fracture_loop:
            update_ssheight(val, phase_random=phase_random)


    def update_ssheight(val, phase_random=False, in_fracture_loop=False):
        ...
        data_x1d    = x1d(slider_nx.val, slider_dx.val)
        data_x1d_km = data_x1d * 1.e-3
        ...
        if not in_fracture_loop:
            # update_fracture(val, phase_random=phase_random)
            # Force full iterations for fracture update, even for fixed phase
            update_fracture(val, phase_random=True)


    def update_fracture(val, phase_random=False):
        ...
        Editted by Noah Day - 2025-11-24
        Update the fracture points and histogram.
        Mimics Fortran wave_frac logic: iterate only if random phases are used.
        """

        global data_x1d, data_x1d_km, data_ssh, data_breaks
        global data_frac_radii, data_frac_hist, data_phase, floe_rad_c

        # Determine number of iterations based on phase type
        loop_max = loop_max_iter if phase_random else 1
        ...
        # Only update SSH once per call (no recursion)
        if data_is_mono:
            data_ssh = ssh_mono(data_x1d, slider_hs.val, slider_fp.val, data_phase[0])
        else:
            data_ssh = ssh(data_x1d, data_wfreq, data_wdfreq, data_wspec, data_phase)
    ...
    slider_nx.on_changed(update_spectrum)
